...
fun main() {
    ...
    println("Total modules:               ${result.stats.totalModules}")
    println("Total protos:                ${result.stats.totalProtos}")
    println("Total messages:              ${result.stats.totalMessages}")
    println("Total enums:                 ${result.stats.totalEnums}")
    println("Avg protos per module:       ${"%.1f".format(result.stats.averageProtosPerModule)}")
    println("Smallest module:             ${result.stats.smallestModule} protos")
    println("Largest module:              ${result.stats.largestModule} protos")
    println("Total module dependencies:   ${result.stats.totalModuleDependencies}")
    println("Avg deps per module:         ${"%.1f".format(result.stats.averageDependenciesPerModule)}")
    println()
    ...
    println("Summary")
    println("=".repeat(80))
    println("✓ Successfully grouped ${result.stats.totalProtos} protos into ${result.stats.totalModules} modules")
    println("✓ No circular dependencies between modules")
    println("✓ Module size range: ${result.stats.smallestModule}-${result.stats.largestModule} protos")
    println("✓ Average module dependencies: ${"%.1f".format(result.stats.averageDependenciesPerModule)}")
    println()
    println("Next steps:")
